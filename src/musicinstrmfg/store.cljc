(ns musicinstrmfg.store
  "SSoT for the musical-instrument-workshop plant-operations
  ...
  self-contained (no external musical-instrument-manufacturing
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             instrument-family/pitch-accuracy/wood-
                             moisture/defect-rate record. `:verified?`
    ...
    - `equipment`         -- a crafting/assembly unit's own record.
    ...
    - `maintenance`       -- a scheduled crafting/assembly-equipment
                             maintenance-window DRAFT against a piece
                             of equipment (`musicinstrmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound instrument shipment
                             DRAFT (`musicinstrmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (musicinstrmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (musicinstrmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :instrument-family :strings
                :lot-number "MUS-2026-001" :pitch-accuracy-cents 5
                :quantity-units 200.0 :wood-moisture-content-percent 8.0
                :defect-rate-percent 1.2
                :verified? true :registered? true
                :shipped-units 40.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :instrument-family :wind
                :lot-number "MUS-2026-002" :pitch-accuracy-cents 8
                :quantity-units 60.0 :wood-moisture-content-percent 7.5
                :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-units 55.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :instrument-family :percussion
                :lot-number "MUS-2026-003" :pitch-accuracy-cents 12
                :quantity-units 300.0 :wood-moisture-content-percent 9.0
...
(defn- sample-equipment []
  {"woodshop-001" {:id "woodshop-001" :kind :woodworking-bench
                    :verified? true :registered? true
                    :last-maintenance-date "2026-05-01"}
   "finishing-002" {:id "finishing-002" :kind :finishing-station
                     :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `musicinstrmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `musicinstrmfg.registry` and
...
(defn sample-data!
  ...
  woodworking bench (schedulable for maintenance), one UNVERIFIED/
  unregistered finishing station (blocks any maintenance scheduling
