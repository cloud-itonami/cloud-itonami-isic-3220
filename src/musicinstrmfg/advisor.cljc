(ns musicinstrmfg.advisor
  "InstrumentAdvisor -- the *contained intelligence node* for the
  musical-instrument-workshop plant-operations coordination actor.

  It normalizes production-batch patches (instrument-family/pitch-
  accuracy-cents/wood-moisture-content-percent/defect-rate), drafts a
  crafting/assembly-equipment maintenance scheduling proposal against
  a piece of equipment, drafts a safety-concern flag, and drafts an
  outbound instrument shipment coordination proposal against a
  ...
  a committed record and NEVER a real crafting/assembly-equipment
  actuation or freight dispatch -- see README `What this actor does
  NOT do`. Every output is censored downstream by
  `musicinstrmfg.governor` before anything touches the SSoT.
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `musicinstrmfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct crafting/
  ...
  `musicinstrmfg.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [musicinstrmfg.registry :as registry]
...
(defn- log-production-batch
  ...
  family, pitch-accuracy-cents, wood-moisture-content-percent, or
  defect-rate, nor its verification status. High confidence, low
...
(defn- schedule-maintenance
  "Draft a crafting/assembly-equipment maintenance-window scheduling
  ...
  `musicinstrmfg.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  "Draft a materials-safety(wood-dust/finish-chemical)/equipment-
  ...
  technicality). See `musicinstrmfg.phase`: no phase ever adds this op
  to a phase's `:auto` set; `musicinstrmfg.governor` also always
...
(defn- coordinate-shipment
  "Draft an outbound instrument shipment coordination proposal against
  a production batch. The advisor passes through the caller's own
  claimed unit quantity -- it does NOT invent one, and
  `musicinstrmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "You are the advisor for a musical-instrument workshop "
       ...
       "equipment or batches. Never propose direct actuation of crafting/"
       "assembly equipment (this actor only proposes, it never executes). "
       "Never misreport shipment quantity."))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `musicinstrmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :musicinstrmfg-advisor-proposal
   :op         (:op request)
