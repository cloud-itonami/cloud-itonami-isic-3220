(ns musicinstrmfg.operation
  "InstrumentOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (InstrumentAdvisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the Musical
  Instrument Workshop Plant Operations Governor (:govern) and the
  ...
  (:require [langgraph.graph :as g]
            ...
            [musicinstrmfg.advisor :as advisor]
            [musicinstrmfg.governor :as governor]
            [musicinstrmfg.phase :as phase]
...
(defn build
  "Compiles an InstrumentOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `musicinstrmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; InstrumentAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Musical Instrument Workshop Plant Operations Governor -- independent censor.
      (g/add-node :govern
