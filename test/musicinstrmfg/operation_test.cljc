(ns musicinstrmfg.operation-test
  "Smoke tests for the compiled InstrumentOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `musicinstrmfg.governor-contract-test`; the Store contract in
  `musicinstrmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "InstrumentOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "woodshop-001" :severity :moderate :description "wood-dust collector filter concern"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
