(ns musicinstrmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control crafting/assembly equipment
  directly... does NOT authorize or execute maintenance/equipment
  operations') implemented faithfully. The single invariant under
  test:

    InstrumentAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Musical Instrument Workshop
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:instrument-family :strings}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :strings (:instrument-family (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-woodshop-unit :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest invalid-instrument-family-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:instrument-family :theremin-experimental}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-instrument-family} (-> (store/ledger db) last :basis)))
    (is (not= :theremin-experimental (:instrument-family (store/batch db "batch-001"))) "fabricated instrument-family never lands in the SSoT")))

(deftest invalid-pitch-accuracy-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:pitch-accuracy-cents 500}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-pitch-accuracy} (-> (store/ledger db) last :basis)))
    (is (not= 500 (:pitch-accuracy-cents (store/batch db "batch-001"))) "fabricated pitch-accuracy-cents never lands in the SSoT")))

(deftest invalid-wood-moisture-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10c" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:wood-moisture-content-percent -5.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-wood-moisture} (-> (store/ledger db) last :basis)))
    (is (not= -5.0 (:wood-moisture-content-percent (store/batch db "batch-001"))) "fabricated wood-moisture-content-percent never lands in the SSoT")))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "woodshop-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
